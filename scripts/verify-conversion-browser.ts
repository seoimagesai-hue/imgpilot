/**
 * Prompt 14 Playwright conversion UI verification.
 * Usage: npx tsx scripts/verify-conversion-browser.ts http://localhost:3000
 */
...
async function main() {
  ...
  if (!isR2Configured()) {
    set("Conversion browser", "Blocked", "R2 not configured");
  ...
  const email = `conv-ui-${stamp}@example.com`;
  const password = `ConvUi-${stamp}-Safe!`;
  const filename = `conv-ui-${stamp}.jpg`;
  ...
  const project = await createOwnedProject(userId, {
    name: `Conv UI ${stamp}`,
    websiteUrl: "https://conv-ui.example",
    description: "conversion browser",
    metadataLanguage: "en",
  });

  const jpeg = await sharp({
    create: {width: 120, height: 80, channels: 3, background: {r: 70, g: 40, b: 110}},
  })
    .jpeg({quality: 90})
    .toBuffer();
  ...
  try {
    {
      ...
      assert(
        "EN summary converted",
        await page.getByText(/Converted derivatives/i).isVisible(),
      );
      await openDetailsByFilename(page, filename);
      const dialog = page.locator("dialog").filter({hasText: /Image details/i});
      assert("EN convert title", await dialog.getByText(/Format conversion/i).isVisible());
      assert("EN WebP option", await dialog.getByText(/^WebP$/i).first().isVisible());
      assert("EN no silent jpeg copy", /never silently converted to JPEG/i.test(await dialog.innerText()));

      const webpRow = dialog.locator("li").filter({hasText: /^WebP/i}).first();
      await webpRow.getByRole("button", {name: /^Generate$/i}).click();
      await dialog.getByRole("button", {name: /Convert format/i}).click();
...

    {
      ...
      assert("UR convert title", await dialog.getByText(/فارمیٹ conversion/i).isVisible());
      await page.close();
    }

    {
      ...
      await openDetailsByFilename(page, filename);
      const dialog = page.locator("dialog").filter({hasText: /Image details/i});
      assert("Mobile convert visible", await dialog.getByText(/Format conversion|WebP|AVIF/i).first().isVisible());
      await page.close();
    }
  ...
  set("Conversion browser", failures.length === 0 ? "Passed" : "Failed");
  console.log("\n=== REPORT ===");
  console.log(JSON.stringify(report, null, 2));
