/**
 * Prompt 13 Playwright resize UI verification.
 * Usage: npx tsx scripts/verify-resize-browser.ts http://localhost:3000
 */
...
async function main() {
  ...
  if (!isR2Configured()) {
    set("Resize browser", "Blocked", "R2 not configured");
  ...
  const email = `resize-ui-${stamp}@example.com`;
  const password = `ResizeUi-${stamp}-Safe!`;
  const filename = `resize-ui-${stamp}.jpg`;
  ...
  const project = await createOwnedProject(userId, {
    name: `Resize UI ${stamp}`,
    websiteUrl: "https://resize-ui.example",
  ...
  try {
    {
      ...
      assert(
        "EN summary resize derivatives",
        await page.getByText(/Resize derivatives/i).isVisible(),
      );
      await openDetailsByFilename(page, filename);
      const dialog = page.locator("dialog").filter({hasText: /Image details/i});
      assert("EN Resize presets title", await dialog.getByText(/Resize presets/i).isVisible());
      assert("EN 512 preset", await dialog.getByText(/512 px/i).first().isVisible());
      assert("EN no upscale copy", /Never upscales/i.test(await dialog.innerText()));
      await dialog.getByRole("button", {name: /^Generate$/i}).first().click();
      await dialog.getByRole("button", {name: /Generate resize/i}).click();
...

    {
      ...
      assert("UR resize title", await dialog.getByText(/resize presets/i).isVisible());
      await page.close();
    }

    {
      ...
      const dialog = page.locator("dialog").filter({hasText: /Image details/i});
      assert(
        "Mobile presets visible",
        await dialog.getByText(/256 px|512 px|1024 px/i).first().isVisible(),
      );
      await page.close();
    }
  ...
  set("Resize browser", failures.length === 0 ? "Passed" : "Failed");
  console.log("\n=== REPORT ===");
  console.log(JSON.stringify(report, null, 2));
