...
type ConnectionListProps = {
  connections: WordpressConnectionSafeDto[];
};
          ...
          <tbody>
            {connections.map((connection) => (
              <tr key={connection.id} className="border-b border-[var(--border)] last:border-b-0">
                ...
                <td className="max-w-[220px] truncate px-4 py-3 text-[var(--muted)]">{connection.siteHost}</td>
                ...
                <td className="px-4 py-3">
                  <Link
                    href={`/dashboard/settings/integrations/wordpress/${connection.id}`}
