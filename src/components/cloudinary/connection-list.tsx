...
type ConnectionListProps = {
  connections: CloudinaryConnectionSafeDto[];
};

/** Table of Cloudinary connections. Mirrors `components/webflow/connection-list.tsx`. */
        ...
        <table className="w-full min-w-[720px] text-sm">
          <thead>
            <tr className="border-b border-[var(--border)] text-left text-xs uppercase tracking-wide text-[var(--muted)]">
              ...
              <th className="px-4 py-3">{t("columnCloudName")}</th>
              <th className="px-4 py-3">{t("columnDelivery")}</th>
            ...
            {connections.map((connection) => (
              <tr key={connection.id} className="border-b border-[var(--border)] last:border-b-0">
                ...
                <td className="max-w-[220px] truncate px-4 py-3 text-[var(--muted)]">
                  {connection.cloudNameSafe || t("notVerifiedYet")}
                </td>
                <td className="px-4 py-3 text-[var(--muted)]">
                  {t(`deliveryTypeValues.${connection.defaultDeliveryType}` as "deliveryTypeValues.upload")}
                </td>
                ...
                <td className="px-4 py-3">
                  <Link
                    href={`/dashboard/settings/integrations/cloudinary/${connection.id}`}
