...
import {
  ResizeWebpHeroUploadCta,
  ResizeWebpResetCta,
} from "@/components/marketing/resize-webp-ctas";
import {
  getResizeWebpCopy,
...
function BenefitIcon({
  ...
  name: ReturnType<typeof getResizeWebpCopy>["benefits"]["cards"][number]["icon"];
}) {
    ...
    case "responsive":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <rect x="14" y="8" width="7" height="11" rx="1.5" stroke="currentColor" strokeWidth="1.75" />
        </svg>
      );
    case "dimensions":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M4 8V4h4M16 4h4v4M20 16v4h-4M8 20H4v-4" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
        </svg>
      );
    case "aspect":
      return (
...
export function ResizeWebpLandingView({
  landing,
  locale,
}: {
    ...
    >
      ...
      <section className="marketing-container pt-16 pb-10 md:pt-[72px] md:pb-12">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:gap-12">
          <div className={`max-w-[820px] space-y-5 ${isRtl ? "text-right" : ""}`}>
            <div className="inline-flex items-center gap-2 rounded-full border border-[var(--border)] bg-white px-3 py-1.5 text-sm font-medium shadow-[var(--shadow-soft)]">
              ...
              >
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="none">
                  <path d="M4 8V4h4M16 4h4v4M20 16v4h-4M8 20H4v-4" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" />
                </svg>
              </span>
              {copy.hero.badge}
            </div>
            ...
            <ResizeWebpHeroUploadCta label={copy.hero.uploadCta} />
          </div>
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/resize-webp-hero.webp"
        ...
        <div className="marketing-container">
          <LandingToolWorkspace
            ...
            presentation={{
              ...
              popularSizes: RESIZE_WEBP_POPULAR_SIZES.map((s) => ({...s})),
              showQualityNote: true,
              qualityNoteHref: copy.compressNote.href,
              qualityNoteBefore: copy.compressNote.before,
              qualityNoteLink: copy.compressNote.link,
              qualityNoteAfter: copy.compressNote.after,
      ...
      <section className={sectionPad}>
        <div className="marketing-container grid items-center gap-10 lg:grid-cols-2 lg:gap-12">
          ...
          <div className={`overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)] ${isRtl ? "lg:order-1" : ""}`}>
            <Image
              src="/illustrations/resize-webp-compare.webp"
      ...
      <section className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          <div className="mx-auto max-w-[820px] space-y-3 text-center">
            ...
            <p className="text-[15px] leading-relaxed text-[var(--muted-foreground)]">{copy.presets.intro}</p>
          </div>
      ...
      <section className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          ...
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/resize-webp-steps.webp"
      ...
      <section className={sectionPad}>
        <div className="marketing-container space-y-8">
            ...
            <table className="min-w-full text-start text-[15px]">
              ...
              <thead className="bg-[#f8fafc] text-sm font-semibold">
                <tr>
                  ...
                  <th scope="col" className="px-4 py-3 sm:px-5">
                    {isRtl ? "کردار" : "Image role"}
                  </th>
                </tr>
              </thead>
              <tbody>
                {copy.guide.tableRows.map((row) => (
                  <tr key={row.device} className="border-t border-[var(--border)]">
                    <th scope="row" className="px-4 py-3.5 font-semibold sm:px-5">
                      {row.device}
                    </th>
                    <td className="px-4 py-3.5 text-[var(--muted-foreground)] sm:px-5">{row.role}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <aside className="mx-auto max-w-[820px] rounded-[18px] border border-[var(--accent)]/20 bg-[var(--accent-soft)] px-5 py-4">
            <p className="font-semibold">{copy.guide.noteTitle}</p>
            <p className="mt-1 text-[15px] leading-relaxed text-[var(--muted-foreground)]">
              {copy.guide.noteBefore}
              <Link
                href={copy.guide.noteHref}
                className="font-semibold text-[var(--accent)] underline-offset-2 hover:underline"
              >
                {copy.guide.noteLink}
              </Link>
              {copy.guide.noteAfter}
            </p>
          </aside>
        </div>
      </section>
        ...
        <div className="marketing-container">
          <div className="rounded-[22px] border border-[var(--border)] bg-[linear-gradient(135deg,#eff6ff_0%,#ffffff_55%,#f5f3ff_100%)] px-6 py-10 text-center shadow-[var(--shadow-soft)] sm:px-10 sm:py-12">
            ...
            <div className="mt-6 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center">
              <ResizeWebpResetCta label={copy.cta.primaryLabel} />
              <Link
