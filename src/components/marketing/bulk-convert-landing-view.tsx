...
import {
  BulkConvertHeroUploadCta,
  BulkConvertLearnMoreCta,
  BulkConvertResetCta,
} from "@/components/marketing/bulk-convert-ctas";
...
function BenefitIcon({
  ...
  name: ReturnType<typeof getBulkConvertCopy>["benefits"]["cards"][number]["icon"];
}) {
  ...
  switch (name) {
    ...
    case "consistent":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M4 8h16M4 12h10M4 16h14" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
        </svg>
      );
    case "zip":
      return (
...
export function BulkConvertLandingView({locale}: {locale: string}) {
  const copy = getBulkConvertCopy(locale);
  const origin = getPublicAppOrigin();
  const path = "/bulk-convert";
  ...
  return (
    ...
    >
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:gap-12">
          <div className={`max-w-[820px] space-y-5 ${isRtl ? "text-right" : ""}`}>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <BulkConvertHeroUploadCta label={copy.hero.uploadCta} />
              <BulkConvertLearnMoreCta label={copy.hero.learnMoreCta} />
            </div>
          </div>
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/bulk-convert-hero.webp"
      ...
      <section id="bulk-convert-formats" className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          <div className="mx-auto max-w-[820px] space-y-3 text-center">
            <Eyebrow>{copy.conversions.eyebrow}</Eyebrow>
            <h2 className="text-2xl font-semibold tracking-tight sm:text-[1.75rem]">{copy.conversions.title}</h2>
            <p className="text-[15px] leading-relaxed text-[var(--muted-foreground)] sm:text-base">
              {copy.conversions.intro}
            </p>
          </div>
          <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {copy.conversions.cards.map((card) => (
      ...
      <section id="bulk-convert-intro" className={sectionPad}>
        <div className="marketing-container grid items-center gap-10 lg:grid-cols-2 lg:gap-12">
          ...
          >
            <Image
              src="/illustrations/bulk-convert-compare.webp"
      ...
      <section className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          ...
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/bulk-convert-steps.webp"
      ...
      <section className={sectionPad}>
        <div className="marketing-container space-y-8">
          ...
          <div>
            <p className="mb-4 text-center text-sm font-semibold text-[var(--muted-foreground)]">
              {copy.guide.tableCaption}
            </p>
            <div className="grid gap-6 md:grid-cols-3">
              {copy.guide.columns.map((column) => (
                <div
                  ...
                  className="rounded-[18px] border border-[var(--border)] bg-white p-6 shadow-[var(--shadow-soft)]"
                >
                  <h3 className="text-lg font-semibold">{column.format}</h3>
          ...
          <ul className="mx-auto max-w-[820px] space-y-3 rounded-[18px] border border-[var(--border)] bg-white p-6 shadow-[var(--shadow-soft)]">
            {copy.guide.recommendations.map((recommendation) => (
              <li key={recommendation} className="flex gap-3 text-[15px] text-[var(--muted-foreground)]">
        ...
        <div className="marketing-container">
          <div className="rounded-[22px] border border-[var(--border)] bg-[linear-gradient(135deg,#eff6ff_0%,#ffffff_55%,#f5f3ff_100%)] px-6 py-10 text-center shadow-[var(--shadow-soft)] sm:px-10 sm:py-12">
            ...
            <div className="mt-6 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center">
              <BulkConvertResetCta label={copy.cta.primaryLabel} />
              <Link
