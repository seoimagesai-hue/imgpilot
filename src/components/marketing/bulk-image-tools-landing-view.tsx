...
import {
  BulkImageToolsHeroUploadCta,
  BulkImageToolsLearnMoreCta,
  BulkImageToolsResetCta,
} from "@/components/marketing/bulk-image-tools-ctas";
...
function BenefitIcon({
  ...
  name: ReturnType<typeof getBulkImageToolsCopy>["benefits"]["cards"][number]["icon"];
}) {
  ...
  switch (name) {
    ...
    case "compress":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M8 3h5l5 5v11a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2z" stroke="currentColor" strokeWidth="1.75" strokeLinejoin="round" />
    ...
    case "resize":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M4 8V4h4M16 4h4v4M20 16v4h-4M8 20H4v-4" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
    case "convert":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <path d="M11 9l2 2-2 2" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      );
    case "zip":
      return (
...
export function BulkImageToolsLandingView({
  locale,
  initialTool,
}: {
  locale: string;
  ...
  return (
    ...
    >
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:gap-12">
          <div className={`max-w-[820px] space-y-5 ${isRtl ? "text-right" : ""}`}>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <BulkImageToolsHeroUploadCta label={copy.hero.uploadCta} />
              <BulkImageToolsLearnMoreCta label={copy.hero.learnMoreCta} />
            </div>
          </div>
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/bulk-image-tools-hero.webp"
        ...
        <div className="marketing-container">
          <BulkToolWorkspace
            ...
            presentation={{
              ...
              statsOutputTemplate: copy.upload.statsOutputTemplate,
              enableExternalReset: true,
              resetToolKey: "bulk-image-tools",
            }}
          />
        </div>
      </section>
      ...
      <section id="bulk-tools-modes" className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          ...
          <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {copy.tools.cards.map((card) => (
              <li key={card.href}>
                <Link
                  href={`/bulk-image-tools?tool=${card.tool}`}
                ...
                >
                  ...
                  <span className="mt-4 text-sm font-medium text-[var(--accent)]">
                    {isRtl ? "اس موڈ میں کھولیں ←" : "Open in hub →"}
                  </span>
                </Link>
              </li>
            ))}
          ...
          <ul className="flex flex-wrap justify-center gap-4 text-sm">
            {copy.tools.cards.map((card) => (
              <li key={`link-${card.href}`}>
                <Link href={card.href} className="font-medium text-[var(--accent)] underline-offset-2 hover:underline">
                  {card.title} landing
                </Link>
              </li>
            ))}
      ...
      <section className={sectionPad}>
        <div className="marketing-container space-y-8">
          ...
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/bulk-image-tools-steps.webp"
      ...
      <section className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container">
          <div className={`mx-auto max-w-[820px] space-y-5 ${isRtl ? "text-right" : ""}`}>
            <Eyebrow>{copy.limits.eyebrow}</Eyebrow>
            <h2 className="text-2xl font-semibold tracking-tight sm:text-[1.75rem]">{copy.limits.title}</h2>
            <ul className="space-y-3 rounded-[18px] border border-[var(--border)] bg-white p-6 shadow-[var(--shadow-soft)]">
              {copy.limits.items.map((item) => (
                <li key={item} className="flex gap-3 text-[15px] leading-relaxed text-[var(--muted-foreground)]">
                  <span
            ...
            <aside className="rounded-[18px] border border-[var(--accent)]/20 bg-[var(--accent-soft)] px-5 py-4">
              <p className="text-[15px] leading-relaxed text-[var(--muted-foreground)]">{copy.limits.note}</p>
            </aside>
          </div>
        </div>
      </section>
        ...
        <div className="marketing-container">
          <div className="rounded-[22px] border border-[var(--border)] bg-[linear-gradient(135deg,#eff6ff_0%,#ffffff_55%,#f5f3ff_100%)] px-6 py-10 text-center shadow-[var(--shadow-soft)] sm:px-10 sm:py-12">
            ...
            <div className="mt-6 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center">
              <BulkImageToolsResetCta label={copy.cta.primaryLabel} />
              <Link
