...
import {
  CompressWebpHeroUploadCta,
  CompressWebpResetCta,
} from "@/components/marketing/compress-webp-ctas";
...
function BenefitIcon({
  ...
  name: ReturnType<typeof getCompressWebpCopy>["benefits"]["cards"][number]["icon"];
}) {
  ...
  switch (name) {
    ...
    case "bandwidth":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path d="M4 18h16M6 14h12M8 10h8M10 6h4" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
        </svg>
      );
    case "mobile":
      return (
...
export function CompressWebpLandingView({
  landing,
  locale,
}: {
  ...
  return (
    ...
    >
      ...
      <section className="marketing-container pt-16 pb-10 md:pt-[72px] md:pb-12">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:gap-12">
          <div className={`max-w-[820px] space-y-5 ${isRtl ? "text-right" : ""}`}>
            ...
            <CompressWebpHeroUploadCta label={copy.hero.uploadCta} />
          </div>
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/compress-webp-hero.webp"
      ...
      <section className={sectionPad}>
        <div className="marketing-container grid items-center gap-10 lg:grid-cols-2 lg:gap-12">
          ...
          <div className={`overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)] ${isRtl ? "lg:order-1" : ""}`}>
            <Image
              src="/illustrations/compress-webp-compare.webp"
      ...
      <section className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container">
          <div className={`mx-auto max-w-[820px] space-y-4 rounded-[22px] border border-[var(--accent)]/20 bg-white p-6 shadow-[var(--shadow-soft)] sm:p-8 ${isRtl ? "text-right" : ""}`}>
            <h2 className="text-2xl font-semibold tracking-tight sm:text-[1.75rem]">{copy.whyCompress.title}</h2>
            <div className="space-y-4 text-[15px] leading-relaxed text-[var(--muted-foreground)] sm:text-base">
              {copy.whyCompress.paragraphs.map((p) => (
                <p key={p.slice(0, 40)}>{p}</p>
              ))}
            </div>
            <p className="font-semibold text-[var(--foreground)]">{copy.whyCompress.improvementsTitle}</p>
            <ul className="grid gap-3 sm:grid-cols-2">
              {copy.whyCompress.points.map((point) => (
      ...
      <section className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          ...
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/compress-webp-steps.webp"
      ...
      <section className={sectionPad}>
        <div className="marketing-container space-y-8">
          <div className={`mx-auto max-w-[820px] space-y-4 ${isRtl ? "text-right" : ""}`}>
            <Eyebrow>{copy.guide.eyebrow}</Eyebrow>
            ...
            <ul className="space-y-3 text-[15px] leading-relaxed text-[var(--muted-foreground)] sm:text-base">
              {copy.guide.topics.map((topic) => (
                <li key={topic.slice(0, 48)} className="flex gap-3">
                  <span aria-hidden="true" className="mt-2 h-1.5 w-1.5 shrink-0 rounded-full bg-[var(--accent)]" />
                  <span>{topic}</span>
                </li>
              ))}
          ...
          <div>
            ...
            <div className="grid gap-6 md:grid-cols-3">
              {copy.guide.recommendations.map((item) => (
                <div
                  key={item.name}
                  className="rounded-[18px] border border-[var(--border)] bg-white p-6 shadow-[var(--shadow-soft)]"
                >
                  <h3 className="text-lg font-semibold">{item.name}</h3>
                  <p className="mt-2 text-[15px] leading-relaxed text-[var(--muted-foreground)]">{item.bestFor}</p>
        ...
        <div className="marketing-container">
          <div className="rounded-[22px] border border-[var(--border)] bg-[linear-gradient(135deg,#eff6ff_0%,#ffffff_55%,#f5f3ff_100%)] px-6 py-10 text-center shadow-[var(--shadow-soft)] sm:px-10 sm:py-12">
            ...
            <div className="mt-6 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center">
              <CompressWebpResetCta label={copy.cta.primaryLabel} />
              <Link
