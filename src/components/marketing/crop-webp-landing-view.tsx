...
import {
  CropWebpHeroUploadCta,
  CropWebpResetCta,
} from "@/components/marketing/crop-webp-ctas";
...
function BenefitIcon({
  ...
  name: ReturnType<typeof getCropWebpCopy>["benefits"]["cards"][number]["icon"];
}) {
  ...
  switch (name) {
    ...
    case "responsive":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <rect x="3" y="4" width="11" height="9" rx="1.5" stroke="currentColor" strokeWidth="1.75" />
          <rect x="15" y="8" width="6" height="10" rx="1.2" stroke="currentColor" strokeWidth="1.75" />
        </svg>
      );
    case "webp":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          <path
            ...
            strokeLinejoin="round"
          />
          <path d="M8 11v2M12 10v4M16 11v2" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
        </svg>
      );
    case "privacy":
      return (
...
export function CropWebpLandingView({
  landing,
  locale,
}: {
    ...
    >
      ...
      <section className="marketing-container pt-16 pb-10 md:pt-[72px] md:pb-12">
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:gap-12">
          <div className={`max-w-[820px] space-y-5 ${isRtl ? "text-right" : ""}`}>
            ...
            <CropWebpHeroUploadCta label={copy.hero.uploadCta} />
          </div>
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/crop-webp-hero.webp"
      ...
      <section className={sectionPad}>
        <div className="marketing-container grid items-center gap-10 lg:grid-cols-2 lg:gap-12">
          ...
          >
            <Image
              src="/illustrations/crop-webp-compare.webp"
      ...
      <section className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          <div className="mx-auto max-w-[820px] space-y-3 text-center">
            ...
            <p className="text-[15px] leading-relaxed text-[var(--muted-foreground)]">{copy.ratios.intro}</p>
          </div>
            ...
            {copy.ratios.cards.map((card) => (
              <li key={card.id}>
                ...
                >
                  ...
                  <p className="mt-1 text-[15px] text-[var(--muted-foreground)]">{card.hint}</p>
                </a>
              </li>
            ))}
          </ul>
      ...
      <section className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          ...
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/crop-webp-steps.webp"
          ...
          <div className={`mx-auto max-w-[820px] space-y-4 ${isRtl ? "text-right" : ""}`}>
            ...
            <aside className="rounded-[18px] border border-[var(--accent)]/20 bg-[var(--accent-soft)] px-5 py-4">
              <p className="text-[15px] leading-relaxed text-[var(--muted-foreground)]">
                ...
                <Link
                  href={copy.guide.noteResizeHref}
                  className="font-semibold text-[var(--accent)] underline-offset-2 hover:underline"
                >
                  {copy.guide.noteResizeLabel}
                </Link>
                {copy.guide.noteMiddle}
                <Link
                  href={copy.guide.noteCompressHref}
                  className="font-semibold text-[var(--accent)] underline-offset-2 hover:underline"
                >
                  {copy.guide.noteCompressLabel}
                </Link>
        ...
        <div className="marketing-container">
          <div className="rounded-[22px] border border-[var(--border)] bg-[linear-gradient(135deg,#eff6ff_0%,#ffffff_55%,#f5f3ff_100%)] px-6 py-10 text-center shadow-[var(--shadow-soft)] sm:px-10 sm:py-12">
            ...
            <div className="mt-6 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center">
              <CropWebpResetCta label={copy.cta.primaryLabel} />
              <Link
