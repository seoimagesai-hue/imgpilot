...
export function CompressJpgResetCta({label}: {label: string}) {
  return (
    <button
      type="button"
      ...
      onClick={() => {
        ...
        dispatchGuestToolReset("compress-jpg");
      }}
    >
      {label}
    </button>
...
export function CompressJpgHeroUploadCta({label}: {label: string}) {
  return (
    <button
      type="button"
      ...
      onClick={() => {
        ...
        window.setTimeout(() => {
          const dropzone = target?.querySelector<HTMLElement>('[role="button"][aria-label]');
          dropzone?.focus();
        }, 350);
      }}
    >
