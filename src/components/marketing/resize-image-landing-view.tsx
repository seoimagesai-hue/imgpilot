...
import {
  ResizeImageHeroUploadCta,
  ResizeImageResetCta,
  ResizeImageViewSizesCta,
} from "@/components/marketing/resize-image-ctas";
import {Breadcrumbs} from "@/components/marketing/landing-sections";
import {JsonLd} from "@/components/marketing/json-ld";
import {
  getResizeImageCopy,
  RESIZE_IMAGE_POPULAR_SIZES,
} from "@/lib/marketing/resize-image-landing-content";
...
function BenefitIcon({
  ...
  name: ReturnType<typeof getResizeImageCopy>["benefits"]["cards"][number]["icon"];
}) {
    ...
    case "responsive":
      return (
        <svg className={common} viewBox="0 0 24 24" fill="none" aria-hidden="true">
          ...
          <path d="M16.5 15.5h2" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" />
        </svg>
      );
    case "social":
      return (
...
export function ResizeImageLandingView({locale}: {locale: string}) {
  ...
  return (
    ...
    >
        ...
        <div className="grid items-center gap-10 lg:grid-cols-[minmax(0,1fr)_minmax(0,0.95fr)] lg:gap-12">
          <div className={`max-w-[820px] space-y-5 ${isRtl ? "text-right" : ""}`}>
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
              <ResizeImageHeroUploadCta label={copy.hero.uploadCta} />
              <ResizeImageViewSizesCta label={copy.hero.viewSizesCta} />
            </div>
          </div>
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/resize-image-hero.webp"
          ...
          <GuestToolWorkspace
            config={{
              ...
              presentation: {
                ...
                popularSizes: RESIZE_IMAGE_POPULAR_SIZES.map((s) => ({...s})),
      ...
      <section id="resize-presets" className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          <div className="mx-auto max-w-[820px] space-y-3 text-center">
            <Eyebrow>{copy.presets.eyebrow}</Eyebrow>
            <h2 className="text-2xl font-semibold tracking-tight sm:text-[1.75rem]">{copy.presets.title}</h2>
            <p className="text-[15px] leading-relaxed text-[var(--muted-foreground)] sm:text-base">
              {copy.presets.intro}
            </p>
          </div>
          <ul className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {copy.presets.cards.map((card) => (
              <li key={card.id}>
                <a
                  href="#tool-workspace"
                ...
                >
                  ...
                  <p className="mt-2 text-[15px] tabular-nums text-[var(--muted-foreground)]">
                    {card.custom
                      ? isRtl
                        ? "اپنی چوڑائی اور اونچائی"
      ...
      <section id="resize-image-tools" className={sectionPad}>
        <div className="marketing-container space-y-8">
          <div className="mx-auto max-w-[820px] space-y-3 text-center">
      ...
      <section id="why-resize" className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container">
      ...
      <section className={sectionPad}>
        <div className="marketing-container space-y-8">
          ...
          <div className="overflow-hidden rounded-[20px] bg-white ring-1 ring-[var(--border)]">
            <Image
              src="/illustrations/resize-image-steps.webp"
      ...
      <section id="resize-guide" className={`${sectionPad} bg-[#f8fafc]`}>
        <div className="marketing-container space-y-8">
          ...
          <div className="overflow-x-auto rounded-[18px] border border-[var(--border)] bg-white shadow-[var(--shadow-soft)]">
            <table className="min-w-full text-start text-[15px]">
              <caption className="sr-only">{copy.guide.caption}</caption>
              <thead className="bg-[#f8fafc] text-sm font-semibold">
                <tr>
                  {copy.guide.columns.map((column) => (
        ...
        <div className="marketing-container">
          <div className="grid gap-10 lg:grid-cols-2 lg:gap-12">
            <div className={`max-w-[820px] space-y-4 ${isRtl ? "text-right" : ""}`}>
              <h2 className="text-2xl font-semibold tracking-tight sm:text-[1.75rem]">{copy.responsive.title}</h2>
              <div className="space-y-4 text-[15px] leading-relaxed text-[var(--muted-foreground)] sm:text-base">
                {copy.responsive.paragraphs.map((p) => (
                  <p key={p.slice(0, 40)}>{p}</p>
                ))}
              </div>
              <aside className="rounded-[18px] border border-[var(--accent)]/20 bg-[var(--accent-soft)] px-5 py-4">
                <p className="text-[15px] leading-relaxed text-[var(--muted-foreground)]">{copy.responsive.note}</p>
              </aside>
            </div>
            <ul className="grid gap-4">
              {copy.responsive.levels.map((level) => (
                <li
                  key={level.title}
        ...
        <div className="marketing-container">
          <div className="rounded-[22px] border border-[var(--border)] bg-[linear-gradient(135deg,#eff6ff_0%,#ffffff_55%,#f5f3ff_100%)] px-6 py-10 text-center shadow-[var(--shadow-soft)] sm:px-10 sm:py-12">
            ...
            <div className="mt-6 flex flex-col items-stretch justify-center gap-3 sm:flex-row sm:items-center">
              <ResizeImageResetCta label={copy.cta.primaryLabel} />
              <Link
