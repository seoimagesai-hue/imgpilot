...
import {PublishStatusBadge, RetryButton, RETRYABLE_STATUSES} from "@/components/webflow/publish-status";

/**
 * DTO for one row of publish history. Deliberately excludes access tokens
 * and any other server-internal fields — only display-safe columns that are
 * already part of `WebflowPublishJobDto`.
 */
...
/** Table of past Webflow publish attempts — used on connection detail and project publish pages. */
      ...
      <table className="w-full min-w-[720px] text-sm">
        <thead>
          <tr className="border-b border-[var(--border)] text-left text-xs uppercase tracking-wide text-[var(--muted)]">
            <th className="px-3 py-2">{t("columnCmsItem")}</th>
        ...
        <tbody>
          {jobs.map((job) => (
            <tr key={job.id} className="border-b border-[var(--border)] last:border-b-0">
              <td className="max-w-[200px] truncate px-3 py-2">{job.cmsItemNameSafe || job.cmsItemId}</td>
