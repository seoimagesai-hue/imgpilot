...
import {createShopifyPublishAction, type ShopifyActionState} from "@/server/shopify/actions";
import type {MetadataLanguage} from "@/server/projects/validation";
import {ProductSearch, type ShopifyProductOption} from "@/components/shopify/product-search";
...
export function PublishForm({
  ...
  const [state, formAction, pending] = useActionState(createShopifyPublishAction, initial);

  const availableLanguages = Object.keys(approvedMetadataByLanguage) as MetadataLanguage[];
  const [connectionId, setConnectionId] = useState(connections[0]?.id ?? "");
  const [selectedProduct, setSelectedProduct] = useState<ShopifyProductOption | null>(null);
  ...
  return (
    <form action={formAction} className="space-y-4 rounded-2xl border border-[var(--border)] bg-white p-6 shadow-sm" noValidate>
      ...
      <div>
        <label htmlFor="shopify-publish-connection" className="mb-1.5 block text-sm font-medium">
          {t("connection")}
        </label>
        <select
          id="shopify-publish-connection"
          name="connectionId"
          required
          disabled={pending}
          value={connectionId}
      ...
      <div>
        <label htmlFor="shopify-publish-derivative" className="mb-1.5 block text-sm font-medium">
          {t("derivative")}
        </label>
        <select
          id="shopify-publish-derivative"
          name="derivativeId"
          disabled={pending}
          defaultValue=""
          className="w-full rounded-xl border border-[var(--border)] px-3 py-2.5"
      ...
      <div>
        <label htmlFor="shopify-publish-language" className="mb-1.5 block text-sm font-medium">
          {t("language")}
        </label>
        <select
          id="shopify-publish-language"
          name="language"
          required
      ...
      <button
        type="submit"
        disabled={pending || !preview || !selectedProduct}
