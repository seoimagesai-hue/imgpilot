...
import {createShopifyConnectionAction, type ShopifyActionState} from "@/server/shopify/actions";
...
export function ConnectionForm({workspaceType, workspaceId}: ConnectionFormProps) {
  ...
  const [state, formAction, pending] = useActionState(createShopifyConnectionAction, initial);
    ...
    >
      ...
      {state.ok && state.connection ? (
        <div role="status" className="rounded-xl border border-emerald-200 bg-emerald-50 px-3 py-2 text-sm text-emerald-800">
          {t("createdNotice")}{" "}
          <Link href={`/dashboard/settings/integrations/shopify/${state.connection.id}`} className="font-medium underline">
            {t("viewDetails")}
          </Link>
        </div>
      ) : null}
      ...
      <div>
        <label htmlFor="shopify-name" className="mb-1.5 block text-sm font-medium">
          {t("name")}
        </label>
        <input
          id="shopify-name"
      ...
      <div>
        <label htmlFor="shopify-shop" className="mb-1.5 block text-sm font-medium">
          {t("shop")}
        </label>
        <input
          id="shopify-shop"
          ...
          className="w-full rounded-xl border border-[var(--border)] px-3 py-2.5"
        />
        <p className="mt-1 text-xs text-[var(--muted)]">{t("shopHint")}</p>
        {state.fieldErrors?.shop ? <p className="mt-1 text-sm text-red-700">{msg(state.fieldErrors.shop)}</p> : null}
      </div>
