...
type ConnectionListProps = {
  connections: ShopifyConnectionSafeDto[];
};
        ...
        <table className="w-full min-w-[640px] text-sm">
          <thead>
            <tr className="border-b border-[var(--border)] text-left text-xs uppercase tracking-wide text-[var(--muted)]">
              ...
              <th className="px-4 py-3">{t("columnShop")}</th>
          ...
          <tbody>
            {connections.map((connection) => (
              <tr key={connection.id} className="border-b border-[var(--border)] last:border-b-0">
                ...
                <td className="max-w-[220px] truncate px-4 py-3 text-[var(--muted)]">{connection.shopDomain}</td>
                ...
                <td className="px-4 py-3">
                  <Link
                    href={`/dashboard/settings/integrations/shopify/${connection.id}`}
