...
import {CreateApiKeyForm} from "@/components/developer/create-api-key-form";
...
export default async function DeveloperApiKeysPage({params}: Props) {
  const {locale: raw} = await params;
  const locale = isAppLocale(raw) ? raw : "en";
  setRequestLocale(locale);
  const session = await requireUser(locale, "/dashboard/settings/developer/keys");
  ...
  const tKeys = await getTranslations("developer.keys");
    ...
    return (
      <main className="p-4 sm:p-6 lg:p-8">
        <header className="mb-6">
          <h1 className="text-2xl font-semibold tracking-tight sm:text-3xl">{tKeys("title")}</h1>
        </header>
  ...
  const apiKeys = await listApiKeys({actorUserId: userId, workspaceType, workspaceId});
  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8">
      <header className="mb-6">
        ...
        <h1 className="mt-2 text-2xl font-semibold tracking-tight sm:text-3xl">{tKeys("title")}</h1>
        <p className="mt-2 max-w-2xl text-[var(--muted)]">{tKeys("subtitle")}</p>
      </header>
      ...
      <div className="space-y-6">
        {canManage ? <CreateApiKeyForm workspaceType={workspaceType} workspaceId={workspaceId} /> : null}
        <ApiKeyList apiKeys={apiKeys} workspaceType={workspaceType} workspaceId={workspaceId} canManage={canManage} />
      </div>
    </main>
  );
}
