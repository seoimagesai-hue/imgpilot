...
import {ConnectionForm} from "@/components/cloudinary/connection-form";
import {ConnectionList} from "@/components/cloudinary/connection-list";

export const dynamic = "force-dynamic";

type Props = {params: Promise<{locale: string}>};

export default async function CloudinaryIntegrationPage({params}: Props) {
  const {locale: raw} = await params;
  const locale = isAppLocale(raw) ? raw : "en";
  setRequestLocale(locale);
  const session = await requireUser(locale, "/dashboard/settings/integrations/cloudinary");
  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="space-y-6">
        {canManage && entitlement?.plan.cloudinaryEnabled ? (
        ...
        {entitlement?.plan.cloudinaryEnabled ? (
          <p className="text-sm text-[var(--muted)]">
            {t("connectionsUsed")}:{" "}
            <span className="font-medium text-[var(--foreground)]">
              {connections.length} / {entitlement.plan.maxCloudinaryConnections}
            </span>
          </p>
        ) : null}
      ...
      <p className="mt-6 text-sm text-[var(--muted)]">
        <Link href="/docs/cloudinary" className="font-medium text-[var(--accent)]">
          {t("docsLink")}
        </Link>
      </p>
    </main>
