...
import {ConnectionForm} from "@/components/wordpress/connection-form";
import {ConnectionList} from "@/components/wordpress/connection-list";

export const dynamic = "force-dynamic";

type Props = {params: Promise<{locale: string}>};

export default async function WordpressIntegrationPage({params}: Props) {
  const {locale: raw} = await params;
  const locale = isAppLocale(raw) ? raw : "en";
  setRequestLocale(locale);
  const session = await requireUser(locale, "/dashboard/settings/integrations/wordpress");
  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8">
      ...
      <div className="space-y-6">
        {canManage && entitlement?.plan.wordpressEnabled ? (
        ...
        {entitlement?.plan.wordpressEnabled ? (
          <p className="text-sm text-[var(--muted)]">
            {t("connectionsUsed")}:{" "}
            <span className="font-medium text-[var(--foreground)]">
              {connections.length} / {entitlement.plan.maxWordpressConnections}
            </span>
          </p>
        ) : null}
      ...
      <p className="mt-6 text-sm text-[var(--muted)]">
        <Link href="/docs/wordpress" className="font-medium text-[var(--accent)]">
          {t("docsLink")}
        </Link>
      </p>
    </main>
