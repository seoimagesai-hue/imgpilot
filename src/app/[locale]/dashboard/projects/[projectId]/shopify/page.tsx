...
import {listRecentPublishJobsForProject} from "@/server/shopify/publish-service";
import {PublishForm, type PublishFormApprovedMetadata} from "@/components/shopify/publish-form";
import {PublishHistory} from "@/components/shopify/publish-history";
...
export default async function ProjectShopifyPublishPage({params, searchParams}: Props) {
  const {locale: raw, projectId} = await params;
  const rawParams = await searchParams;
  const locale = isAppLocale(raw) ? raw : "en";
  setRequestLocale(locale);
  const session = await requireUser(locale, `/dashboard/projects/${projectId}/shopify`);
  const userId = session.user.id;

  const idParsed = projectIdSchema.safeParse(projectId);
  if (!idParsed.success) notFound();

  const project = await getOwnedProject(userId, idParsed.data, "shopify.publish");
  if (!project) notFound();

  const t = await getTranslations("shopify");
  const tp = await getTranslations("projects");
  const tPublish = await getTranslations("shopify.publish");
    ...
    <main className="p-4 sm:p-6 lg:p-8">
      ...
      {connections.length === 0 ? (
        <div className="rounded-2xl border border-[var(--border)] bg-white p-6 text-sm text-[var(--muted)] shadow-sm">
          {t("emptyText")}{" "}
          <Link href="/dashboard/settings/integrations/shopify" className="font-medium text-[var(--accent)]">
            {t("createTitle")}
          </Link>
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[280px_1fr]">
          <form method="get" className="h-fit rounded-2xl border border-[var(--border)] bg-white p-4 shadow-sm">
            <label htmlFor="shopify-image-picker" className="mb-1.5 block text-sm font-medium">
              {tPublish("imagePicker")}
            </label>
            <select
              id="shopify-image-picker"
              name="imageId"
          ...
          <div className="space-y-6">
            {selectedRow ? (
              <PublishForm
                ...
                connections={activeConnections.map((c) => ({id: c.id, name: c.name, shopDomain: c.shopDomain}))}
