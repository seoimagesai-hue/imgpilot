...
import {BatchProgress} from "@/components/ai-batches/batch-progress";
...
export default async function AiBatchDetailPage({params}: Props) {
  ...
  const session = await requireUser(locale, `/dashboard/projects/${projectId}/ai-batches/${batchId}`);
  ...
  return (
    <main className="p-4 sm:p-6 lg:p-8">
      <header className="mb-6">
        ...
        <h1 className="mt-1 text-2xl font-semibold tracking-tight sm:text-3xl">{t("detail.pageTitle")}</h1>
      ...
      <BatchProgress projectId={project.id} batchId={batchId} initialBatch={result.batch} />
    </main>
  );
}
